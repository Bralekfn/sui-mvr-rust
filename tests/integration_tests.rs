...
async fn test_resolver_configuration() {
    ...
    assert_eq!(mainnet_resolver.config().cache_ttl, Duration::from_secs(3600));
    ...
    assert_eq!(custom_resolver.config().cache_ttl, Duration::from_secs(1800));
...
async fn test_static_overrides() {
    ...
    let result = resolver.resolve_type("@test/package::TestType").await.unwrap();
...
async fn test_batch_operations() {
    ...
    assert_eq!(results.get("@batch/pkg1::Type1"), Some(&"0x111::module::Type1".to_string()));
    assert_eq!(results.get("@batch/pkg2::Type2"), Some(&"0x222::module::Type2".to_string()));
...
async fn test_package_name_validation() {
    ...
    for invalid_name in invalid_package_names() {
        ...
        assert!(result.is_err(), "Should reject invalid package name: {}", invalid_name);
    ...
    for valid_name in valid_package_names() {
        ...
        if valid_name == "@suifrens/core" || valid_name == "@test/pkg" {
            ...
            assert!(result.is_ok(), "Should accept valid package name: {}", valid_name);
        }
    }
}
...
async fn test_type_name_validation() {
    ...
    for invalid_type in invalid_type_names() {
        ...
        assert!(result.is_err(), "Should reject invalid type name: {}", invalid_type);
    ...
    let resolver_with_overrides = create_test_resolver();
    let result = resolver_with_overrides.resolve_type("@suifrens/core::suifren::SuiFren").await;
...
async fn test_concurrent_operations() {
    ...
    for result in results {
        ...
        assert!(resolution_result.is_ok(), "Concurrent resolution should succeed");
...
async fn test_error_types_and_properties() {
    ...
    let rate_limited = MvrError::RateLimitExceeded { retry_after_secs: 60 };
...
async fn test_cache_statistics() {
    ...
    let _ = resolver.resolve_package("@suifrens/accessories").await.unwrap();
...
async fn test_comprehensive_workflow() {
    ...
    let suifren_type = resolver.resolve_type("@suifrens/core::suifren::SuiFren").await.unwrap();
...
async fn test_performance_comparison() {
    ...
    println!("Individual: {:?}, Batch: {:?}", individual_duration, batch_duration);
}
...
// 4. Mock the HTTP client for deterministic testing_package("@concurrent/pkg2".to_string(), "0x222".to_string())
        .with_package("@concurrent/pkg3".to_string(), "0x333".to_string());
    ...
    let tasks = vec![
        tokio::spawn({
            let resolver = resolver.clone();
            async move { resolver.resolve_package("@concurrent/pkg1").await }
        }),
        tokio::spawn({
            let resolver = resolver.clone();
            async move { resolver.resolve_package("@concurrent/pkg2").await }
        }),
        tokio::spawn({
            let resolver = resolver.clone();
            async move { resolver.resolve_package("@concurrent/pkg3").await }
        }),
    ];
...
fn create_test_resolver() -> MvrResolver {
    let overrides = MvrOverrides::new()
        ...
        .with_package("@suifrens/accessories".to_string(), "0x987654321".to_string())
        .with_type("@suifrens/core::suifren::SuiFren".to_string(), 
                   "0x123456789::suifren::SuiFren".to_string())
        .with_type("@suifrens/core::bullshark::Bullshark".to_string(), 
...
async fn test_comprehensive_workflow() {
    ...
    // Test cache statistics
    let stats = resolver.cache_stats().unwrap();
    assert!(stats.total_entries >= 0); // Should have some entries from resolutions
...
// Note: To run these tests with a real MVR server, you would need to:
// 1. Set up environment variables for test endpoints
// 2. Handle rate limiting and network timeouts appropriately
// 3. Use actual package names that exist in the MVR registry