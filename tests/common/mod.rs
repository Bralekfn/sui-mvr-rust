...
pub fn create_test_resolver() -> MvrResolver {
    let overrides = MvrOverrides::new()
        ...
        .with_type("@test/package::TestType".to_string(), 
...
pub fn create_batch_test_overrides() -> MvrOverrides {
    ...
    types.insert("@batch/pkg1::Type1".to_string(), "0x111::module::Type1".to_string());
    types.insert("@batch/pkg2::Type2".to_string(), "0x222::module::Type2".to_string());
    types.insert("@batch/pkg3::Type3".to_string(), "0x333::module::Type3".to_string());
...
pub fn assert_valid_address(address: &str) {
    assert!(address.starts_with("0x"), "Address should start with 0x: {}", address);
    assert!(address.len() >= 3, "Address should be longer than just 0x: {}", address);
    
    // Check that the rest are valid hex characters
    let hex_part = &address[2..];
    for c in hex_part.chars() {
        assert!(c.is_ascii_hexdigit(), "Invalid hex character in address: {}", address);
    }
}

/// Assert that a type signature is valid
pub fn assert_valid_type_signature(type_sig: &str) {
    assert!(type_sig.contains("::"), "Type signature should contain :: separator: {}", type_sig);
    ...
    assert!(parts.len() >= 3, "Type signature should have at least address::module::Type: {}", type_sig);
}

/// Utility for testing error properties
pub fn test_error_properties(error: &MvrError, expected_retryable: bool, expected_client_error: bool) {
    assert_eq!(error.is_retryable(), expected_retryable, 
               "Error retryable property mismatch for: {:?}", error);
    assert_eq!(error.is_client_error(), expected_client_error, 
