...
use sui_sdk::{
    types::{
        ...
        transaction::{TransactionData, Argument},
        TypeTag,
    },
...
fn load_production_config() -> Result<MvrConfig> {
    ...
    if let Ok(overrides_json) = std::env::var("MVR_OVERRIDES") {
        if let Ok(overrides) = MvrOverrides::from_json(&overrides_json) {
            ...
            println!("📝 Loaded {} package overrides from environment", overrides.packages.len());
        }
    }
...
async fn resolve_package_with_retry(
    ...
    loop {
        match resolver.resolve_package(package_name).await {
            ...
            Err(e) => {
                println!("❌ Failed to resolve '{}' after {} attempts: {}", 
...
impl ProductionApp {
    ...
    pub async fn execute_defi_batch(&self, operations: Vec<DefiOperation>) -> Result<Vec<String>> {
        println!("🔄 Executing batch DeFi operations ({} operations)...", operations.len());
        
        let mut results = Vec::new();
        
        for (i, operation) in operations.into_iter().enumerate() {
            println!("  [{}/{}] Processing operation: {:?}", i + 1, results.len() + 1, operation);
            
            let result = match operation {
                DefiOperation::AddLiquidity { pool_package, amount_a, amount_b } => {
                    let package_address = resolve_package_with_retry(&self.resolver, &pool_package, 3).await?;
                    let package_id = ObjectID::from_hex_literal(&package_address)?;
                    
                    // Build add liquidity transaction
                    let ptb = self.resolver.build_move_call_transaction(
                        &pool_package,
                        "pool",
                        "add_liquidity",
                    ...
                    format!("Add liquidity: {} + {} to pool {}", amount_a, amount_b, package_id)
                }
                
                DefiOperation::Swap { dex_package, from_amount, to_token_type } => {
                    let package_address = resolve_package_with_retry(&self.resolver, &dex_package, 3).await?;
                    let package_id = ObjectID::from_hex_literal(&package_address)?;
                    ...
                    format!("Swap {} for {} via DEX {}", from_amount, to_token_type, package_id)
                }
            };
...
pub async fn handle_mvr_error(error: &MvrError, package_name: &str) -> Option<String> {
    match error {
        MvrError::PackageNotFound(_) => {
            println!("🔄 Package '{}' not found in MVR, checking fallback mappings...", package_name);
            
            // In production, you might have a fallback registry
            match package_name {
                "@suifrens/core" => {
                    let fallback = "0xee496a0cc04d06a345982ba6697c90c619020de9e274408c7819f787ff66e1a1";
                    println!("✅ Using fallback address for {}: {}", package_name, fallback);
        ...
        MvrError::Timeout { timeout_secs } => {
            println!("⏰ Request timeout after {}s, implementing exponential backoff...", timeout_secs);
            None // Caller should retry
        }
        _ => {
...
async fn main() -> Result<()> {
    ...
    println!("   MVR Responsive: {}", if health.mvr_responsive { "✅" } else { "❌" });
    #[cfg(feature = "sui-integration")]
    println!("   Sui Connectivity: {}", if health.sui_connectivity { "✅" } else { "❌" });
    println!("   Cache Utilization: {:.1}%", health.cache_utilization * 100.0);
    ...
    let demo_overrides = MvrOverrides::new()
        .with_package("@myapp/defi".to_string(), "0x123456789abcdef123456789abcdef123456789abcdef123456789abcdef1234".to_string())
        .with_package("@myapp/nft".to_string(), "0xabcdef123456789abcdef123456789abcdef123456789abcdef123456789abcdef".to_string())
        .with_package("@common/utils".to_string(), "0x987654321fedcba987654321fedcba987654321fedcba987654321fedcba9876".to_string());
    ...
    println!("✅ Resolved {} critical packages in {:?}", resolved.len(), duration);
