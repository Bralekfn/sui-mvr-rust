...
async fn main() -> Result<()> {
    ...
    let sui_client: SuiClient = SuiClientBuilder::default().build_testnet().await?;

    println!("✅ Connected to Sui testnet");
    println!("   RPC version: {}", sui_client.api_version());
    println!(
        "   Chain identifier: {:?}",
        sui_client.read_api().get_chain_identifier().await?
    );
    ...
    let overrides = MvrOverrides::new()
        ...
        .with_type(
            ...
            "0xee496a0cc04d06a345982ba6697c90c619020de9e274408c7819f787ff66e1a1::suifren::SuiFren"
                .to_string(),
        );
    ...
    let type_signature = mvr_resolver
    ...
    let ptb = build_mvr_transaction(&mvr_resolver, "@suifrens/core", "mint", "new_suifren").await?;
    ...
    let package_names = vec!["@suifrens/core", "@example/defi"];
...
mod tests {
    ...
    async fn test_mvr_target_resolution() {
        let resolver = MvrResolver::testnet().with_overrides(MvrOverrides::new().with_package(
            "@test/package".to_string(),
            "0x123456789abcdef123456789abcdef123456789abcdef123456789abcdef1234".to_string(),
        ));

        let (package_id, module, function) =
            resolve_mvr_target(&resolver, "@test/package::module::function")
                .await
                .unwrap();
